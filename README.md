...
- ブラウザ音声合成による読み上げ
- ページ内メディアの一括停止・ミュート
- 12項目の簡易アクセシビリティチェック（問題候補と目視確認項目の提示）
- 日本語／英語切り替え
- 表示設定のブラウザ内保存

簡易チェックは、画像の`alt`不足、見出し階層、ページタイトル、リンク名、フォーカス順に影響する指定などを検出します。コンテンツの意味やページ間の一貫性は自動判定できないため、結果が0件でもキーボードとスクリーンリーダーによる確認が必要です。
